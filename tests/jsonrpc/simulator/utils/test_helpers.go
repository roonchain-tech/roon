...
func NewERC20FilterLogs(rCtx *types.RPCContext, isGeth bool) (ethereum.FilterQuery, string, error) {
	...
	// Create filter on roon
	args, err := ToFilterArg(fErc20Transfer)
	if err != nil {
		return fErc20Transfer, "", fmt.Errorf("failed to create filter args: %w", err)
	}
	var roonFilterID string
	if err = ethCli.RPCClient().CallContext(rCtx, &roonFilterID, "eth_newFilter", args); err != nil {
		return fErc20Transfer, "", fmt.Errorf("failed to create filter on roon: %w", err)
	}

	return fErc20Transfer, roonFilterID, nil
}

// Standard dev account addresses (matching roon genesis accounts)
...
func DeployContract(rCtx *types.RPCContext, contractByteCode []byte, isGeth bool) (addr common.Address, txHash string, blockNum *big.Int, err error) {
	...
	fmt.Printf("Deploying ERC20 to roon using dev0 (%s)...\n", fromAddress.Hex())

	roonTxHash, err := deployContractViaDynamicFeeTx(ethCli.Client, privateKey, contractByteCode)
	if err != nil {
		return common.Address{}, "", nil, err
	} else {
		addr, blockNum, err = waitForContractDeployment(ethCli.Client, roonTxHash, 30*time.Second)
		if err != nil {
			return common.Address{}, "", nil, err
		}
	}

	fmt.Printf("✓ roon deployment successful: %s\n", addr.Hex())
	return addr, roonTxHash, blockNum, nil
}
...
func waitForContractDeployment(client *ethclient.Client, txHashStr string, timeout time.Duration) (common.Address, *big.Int, error) {
	fmt.Printf("Waiting for roon deployment (tx: %s)...\n", txHashStr)
