...
func VerifyTokenBalances(rCtx *types.RPCContext) error {
	...
	for _, privateKeyHex := range accounts {
		...
		// Get balance on roon
		roonBalance, err := getTokenBalance(rCtx, addr, false)
		if err != nil {
			return fmt.Errorf("failed to get roon balance for %s: %w", addr.Hex(), err)
		}
		...
		// Compare balances
		if roonBalance.Cmp(gethBalance) != 0 {
			return fmt.Errorf("balance mismatch for %s: roon=%s, geth=%s",
				addr.Hex(), roonBalance.String(), gethBalance.String())
		}

		readableBalance := new(big.Int).Div(roonBalance, big.NewInt(1e18))
