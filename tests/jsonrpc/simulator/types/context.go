...
// ComparisonResult holds the result of comparing API response structures between roon and geth
type ComparisonResult struct {
	Method         string            `json:"method"`
	EvmdType       string            `json:"roon_type"`
	GethType       string            `json:"geth_type"`
	EvmdStructure  map[string]string `json:"roon_structure,omitempty"` // Field names -> types
	...
	EvmdError      string            `json:"roon_error,omitempty"`
...
func NewRPCContext(conf *config.Config) (*RPCContext, error) {
	// Connect to the primary Ethereum client (roon)
...
func (rCtx *RPCContext) CompareRPCCall(method string, params ...interface{}) *ComparisonResult {
	...
	// Call roon
	var roonResponse interface{}
	roonErr := rCtx.Evmd.RPCClient().CallContext(context.Background(), &roonResponse, method, params...)
	if roonErr != nil {
		result.EvmdError = roonErr.Error()
	}
	...
	// Compare errors
	result.ErrorsMatch = (roonErr == nil && gethErr == nil) ||
		(roonErr != nil && gethErr != nil)

	// Compare structure and types if both succeeded
	if roonErr == nil && gethErr == nil {
		result.EvmdType = rCtx.getTypeDescription(roonResponse)
		result.GethType = rCtx.getTypeDescription(gethResponse)
		result.TypeMatch = result.EvmdType == result.GethType

		result.EvmdStructure = rCtx.analyzeStructure(roonResponse)
...
func (rCtx *RPCContext) CompareRPCCallWithProvider(method string, paramProvider ParameterProvider) *ComparisonResult {
	...
	roonParams := paramProvider(false) // false = roon
	gethParams := paramProvider(true)  // true = geth

	// Call roon
	var roonResponse interface{}
	roonErr := rCtx.Evmd.RPCClient().CallContext(context.Background(), &roonResponse, method, roonParams...)
	if roonErr != nil {
		result.EvmdError = roonErr.Error()
	}
	...
	if roonErr == nil && gethErr == nil {
		...
		if roonErr != nil && gethErr == nil {
			result.Differences = []string{"roon request failed, geth succeeded - cannot compare structures"}
		} else if roonErr == nil && gethErr != nil {
			result.Differences = []string{"geth request failed, roon succeeded - cannot compare structures"}
		} else {
...
func (rCtx *RPCContext) findStructuralDifferences(roonType, gethType string, roonStruct, gethStruct map[string]string) []string {
	var differences []string

	// Type comparison
	if roonType != gethType {
		differences = append(differences, fmt.Sprintf("Root type mismatch: roon=%s, geth=%s", roonType, gethType))
	}

	// Find missing fields in roon
	for field, fieldType := range gethStruct {
		if _, exists := roonStruct[field]; !exists {
			differences = append(differences, fmt.Sprintf("Missing field in roon: %s (%s)", field, fieldType))
		}
	}

	// Find extra fields in roon
	for field, fieldType := range roonStruct {
		if _, exists := gethStruct[field]; !exists {
			differences = append(differences, fmt.Sprintf("Extra field in roon: %s (%s)", field, fieldType))
		}
	}

	// Find type mismatches
	for field, roonFieldType := range roonStruct {
		if gethFieldType, exists := gethStruct[field]; exists && roonFieldType != gethFieldType {
			differences = append(differences, fmt.Sprintf("Type mismatch for %s: roon=%s, geth=%s", field, roonFieldType, gethFieldType))
		}
	}
...
// ParameterProvider is a function that provides parameters for roon and geth separately
