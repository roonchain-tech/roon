...
func Setup() (*types.RPCContext, error) {
	...
	log.Printf("Created filter for ERC20 transfers: roon=%s, geth=%s\n", rCtx.Evmd.FilterID, rCtx.Geth.FilterID)
...
// deployContracts deploys the ERC20 contract to both roon and geth
...
// mintTokensOnBothNetworks distributes ERC20 tokens to specified accounts on both roon and geth
func mintTokensOnBothNetworks(rCtx *types.RPCContext) error {
	...
	// Distribute on roon (from dev0 who deployed the contract)
	fmt.Printf("Distributing tokens on roon (contract: %s)...\n", rCtx.Evmd.ERC20Addr.Hex())
	roonReceipts, err := distributeTokensOnNetwork(rCtx, distributionTargets, config.Dev0PrivateKey, false)
	if err != nil {
		return fmt.Errorf("failed to distribute tokens on roon: %w", err)
	}
	...
	roonSuccess := 0
	gethSuccess := 0
	for _, receipt := range roonReceipts {
		if receipt.Status == 1 {
			roonSuccess++
		}
	}
	...
	fmt.Printf("\nDistribution summary: roon (%d/%d), geth (%d/%d)\n",
		roonSuccess, len(roonReceipts), gethSuccess, len(gethReceipts))

	if roonSuccess != len(distributionTargets) || gethSuccess != len(distributionTargets) {
...
func newFilter(rCtx *types.RPCContext) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create roon filter: %w", err)
	}
