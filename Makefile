...
# evmd module root lives under ./roon (has its own go.mod)
EVMD_DIR      := roon
...
test-evmd:
	@cd roon && go test -count=1 -race -tags=test -mod=readonly $(ARGS) $(EXTRA_ARGS) $(PACKAGES_EVMD)
...
test-all:
	...
	@cd roon && go test -race -tags=test -mod=readonly -timeout=15m $(PACKAGES_EVMD)
