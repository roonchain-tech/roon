...
import (
    ...
    roonebug "github.com/cosmos/evm/client/debug"
    "github.com/cosmos/evm/crypto/hd"
    "github.com/cosmos/evm/roon"
    "github.com/cosmos/evm/roon/config"
...
// NewRootCmd creates a new root command for roon. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	// we "pre"-instantiate the application for getting the injected/configured encoding configuration
	// and the CLI options for the modules
	// add keyring to autocli opts
    tempApp := roon.NewExampleApp(
        log.NewNopLogger(),
        dbm.NewMemDB(),
        nil,
        true,
...
func initRootCmd(rootCmd *cobra.Command, evmApp *roon.EVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	if height != -1 {
		exampleApp = roon.NewExampleApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = roon.NewExampleApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID)) // TODO:VLAD - Remove // TODO:VLAD - Remove appoptions and evmchainid
	}
